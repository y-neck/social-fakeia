...
export function getChartConfig(rawData) {
    ...
    const types = Object
        .keys(actorTypeLut)
        .map((key) => actorTypeLut[key]);
    const roles = Object
        .keys(actorRoleLut)
        .map((key) => actorRoleLut[key]);
    // build datasets per role with value per type each value is the relative
    // frequency of posts with that type and role
