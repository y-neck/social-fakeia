...
export function getChartConfig(rawData) {
    ...
    Object
        ...
        .forEach(([key, count]) => {
            ...
            const [postLabel,
                visualLabel] = key.split("||");
            // find code ov visualLabel
            const visualCode = Object
                .entries(visualTopicLut)
                .find(([, vl]) => vl === visualLabel)
                ?.[0];
            // find super_topic of visualCode
            const supLabel = Object
                .entries(superLut)
                .find(([, codes]) => codes.includes(visualCode))
                ?.[0] ?? "Nicht erkennbar";
    ...
    const datasets = superTopics.map((sup, i) => ({
        ...
        data: postTopics.map((postTopic) => {
            ...
            const rowTotal = Object
                .values(matrix[postTopic])
                .reduce((a, b) => a + b, 0);
            // percent of overall (or you could do percent-of-row only)
            const percent = rowTotal === 0
                ? 0
